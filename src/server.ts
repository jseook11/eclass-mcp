...
export function createEclassServer({ username, session, fileCache, examCache, handoffBaseUrl }: EclassServerContext): Server {
  const server = new Server(
    ...
    {
      capabilities: { tools: {} },
      instructions: "Use local/cache tools before network tools when possible. eclass_submit_assignment defaults to dry_run=true; only set dry_run=false after the user explicitly confirms the exact course, assignment, and submission content in the current conversation.",
    },
  );
  ...
  return {
    tools: buildToolList([
      ...
      },
      {
        ...
        description: '[네트워크] 강의 자료 목록/메타데이터를 가져옵니다 (모듈, 파일함, 강의자료실, 외부도구). 강의자료는 주차학습(modulebuilder), LearningX 강의자료실(courseresource), 공지 첨부(announcements), Canvas 모듈/외부 링크(modules/external)에 분산될 수 있으므로 한 source에서 자료를 찾았어도 다른 source를 생략하지 말고 결과를 합쳐 확인합니다. 권장 1차 조회는 modulebuilder, courseresource, announcements, modules, external이며, Canvas 기본 파일함(files)은 Files 탭이 노출되거나 사용자가 명시적으로 요청한 경우에만 마지막으로 조회합니다. 중앙대 학생 계정에서 files 401은 권한 거부일 수 있으므로 토큰 만료로 보고 재로그인하지 않습니다. 주차학습이 아직 시작되지 않은 경우 `errors[].error_code=LEARNING_PERIOD_NOT_STARTED`와 함께 학습 시작 이후 재조회하라는 안내를 반환하며 `not_open` placeholder URL은 반환하지 않습니다. 이 도구는 파일 본문을 다운로드하거나 ChatGPT에 첨부하지 않습니다. 파일은 eclass_download_file/eclass_download_materials_batch로 MCP 서버 로컬 캐시에 받은 뒤, ChatGPT가 읽어야 하면 eclass_file_handoff로 공개 /files/<token> URL을 별도 발급해야 합니다. 반환값은 { ok, course_id, sources, materials, errors, warnings } JSON 객체이며, 일부 source 실패 시 성공한 자료와 실패 정보를 함께 반환합니다.',
        inputSchema: {
          type: 'object',
          properties: {
            course_id: { type: 'number', description: '강의 ID' },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  return normalizeToolResult(result as Parameters<typeof normalizeToolResult>[0]);
});
