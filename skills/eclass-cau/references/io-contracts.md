...
두 도구의 기본 목록은 현재 학기 일반 교과목이다. 이전 학기나 예방/의무교육이 대상이면
`eclass_get_courses`의 `scope: "all"` 또는 `scope: "training"`을 명시한다.
...
- **전수 처리**: `eclass_get_courses_cached`가 반환한 **현재 학기 스냅샷의 모든**
  `course_id`를 빠짐없이
