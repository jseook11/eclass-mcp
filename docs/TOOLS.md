...
MCP 서버 로컬 캐시에 다운로드된 파일 기록 검색. **네트워크 호출 없음** (로컬 캐시 DB만). 파일 본문은 반환하지 않는다.
...
- 출력(성공): `{ ok: true, video_id, display_name, local_path, size_bytes, skipped, strategy: 'ocs_uniplayer_mp4', handoff_note }`
- 출력(실패): `{ ok: false, error_code, message, retryable, next_action?, debug? }`
...
MCP 서버 로컬 캐시의 다운로드 기록 원본 목록. 파일 본문은 반환하지 않는다.
...
다운로드 현황 요약. 로컬 DB만 사용 (네트워크 없음). 파일 본문은 반환하지 않는다.

- 입력: `{ course_id?: number }`
- 출력: course_id 없으면 `{ mode: 'summary', courses: [...], total_file_count, total_size_bytes, handoff_note }`, 있으면 `{ mode: 'detail', downloads: [...], handoff_note, ... }`
