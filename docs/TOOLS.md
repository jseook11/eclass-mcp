...
- **`eclass://download/<file_id>`** 항목은 특별 취급한다. HTTP transport에서는 `text`/`url`/`metadata.download_url`에 **파일 URL**(`/files/<token>`)을 담아 반환한다. ChatGPT가 브라우징으로 이 URL을 직접 열어 읽게 하려면 `/files` 경로가 공개 도달 가능해야 하며, 로컬 브라우저 다운로드만 필요하면 같은 머신에서 localhost 링크를 열면 된다(서버는 MCP `resources` 기능을 노출하지 않으므로 ChatGPT가 `read_resource`로 파일을 읽으려 하면 실패한다 — 링크 경로가 정상 경로다). stdio에서는 메타데이터 JSON(`local_path` 포함)을 반환한다.
